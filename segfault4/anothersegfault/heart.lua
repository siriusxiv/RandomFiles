...
function getCameraPosition(frame)
    return {1000*math.sin(2*frame/180*2*math.pi), 1000*math.cos(2*frame/180*2*math.pi), 0}
end
...
local instVolumeModel3 = CreateVolumeModel({volume=instSPHLoader1:VolumeData(), translate={0,0,0}, rotate={0,0,0}, scale={1,1,6}, shadername='def_volume_raymarch_texture.frag.c', Uniform={Uniform0=instUniformFloat5:uniform(), Uniform1=instUniformFloat6:uniform(), Uniform2=instUniformFloat7:uniform()}})
local instCreateCamera2 = CreateCamera({position=getCameraPosition(i), target={0,0,0}, up={0,0,-1}, fov = 70, screensize={1024,1024}, filename=output .. frameToString(i) .. '.png'})
